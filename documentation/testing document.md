...
2. What has been tested, how was this done?<br />
   - [x] **Time complexity tests'** results, achieved time complexity etc. are in [the specification document](https://github.com/jakubgrad/Aineopintojen-harjoitusty-Algoritmit-ja-teko-ly-/blob/main/documentation/specification%20document.md)
   ...
   - [x] There is a few correctness tests for **JPS** that test whether the distance returned by **JPS** is correct or not. The tests use `arena.map` and approximated path found by hand. 
   - [x] Visual testing was done for both of the algorithms through the convenient feature of animating in the UI. Anything more complicated would require some precalculated values and bigger maps.<br />
   - [ ] There is no time or space tests. 
