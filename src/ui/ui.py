...
class UI:
    ...
    def create_widgets(self):
        ...
        self.btn_open = tk.Button(
            self.frm_buttons, text="Choose map", command=self.open_file)
        self.btn_dijkstra = tk.Button(
            self.frm_buttons, text="Run Dijkstra", command=self.run_dijkstra)
        self.btn_jps = tk.Button(
            self.frm_buttons, text="Run JPS", command=self.run_jps)

        self.lbl_start_coordinates = tk.Label(
            self.frm_buttons, text="Start Coordinates:")
        self.lbl_goal_coordinates = tk.Label(
            self.frm_buttons, text="Goal Coordinates:")
        ...
        self.btn_jps.grid(row=6, column=0, columnspan=2,
                          sticky="ew", padx=5, pady=5)
    ...
    def run_algorithm(self, dijkstra=False, jps=False):
        ...
        def callback(new_text): return self.update_text(new_text)
        slides = []
