...
class UI:
    ...
    def update_counter(self, number):
        ...
        #print(f"self.max_counter{self.max_counter}")
        if 0 <= number <= self.max_counter:
            self.counter = number
            #print(self.counter)
            self.update_text(self.slides[number])
            self.lbl_update_counter.config(text=f"Browse slides from 0 to {self.max_counter}, current is {self.counter}")
            return True
        return False
    ...
    def create_widgets(self):
        ...
        self.lbl_error_log = tk.Label(
            self.frm_buttons, text="Error log")

        self.txt_error_log = tk.Text(
            self.frm_buttons, font=("Consolas", 6), wrap="word", height=30,width=60)
        self.txt_error_log.config(state=tk.DISABLED)  
        ...
        self.lbl_error_log.grid(
            row=16, column=0, sticky="w", padx=5, pady=5)

        self.txt_error_log.grid(
    ...
    def run_default_dijkstra(self):
        self.run_algorithm(dijkstra=True,jps=False, start="4,4", goal="7,7",map=default_dijkstra)

    def update_error_log(self, message):
        self.txt_error_log.config(state=tk.NORMAL)
        self.txt_error_log.insert("1.0", message + "\n")
        self.txt_error_log.config(state=tk.DISABLED)

    def run_default_jps(self):
        self.run_algorithm(dijkstra=False,jps=True, start="1,1", goal="4,7",map=default_jps)
    ...
    def run_algorithm(self, dijkstra=False, jps=False, start=False, goal=False, map=False):
        ...
        slides = []
        self.algorithm_service.run_algorithm(
        ...
        self.lbl_update_counter.config(text=f"Browse slides from 0 to {self.max_counter}")


        print(self.slides[40])
        print("above slides 40")
        return
        if jps:
            self.algorithm_service.run_algorithm(
                start, goal, map, slides, dijkstra=dijkstra, jps=jps, visualization=False)
            for idx, slide in enumerate(slides):
                self.window.after(idx * 400, lambda s=slide: self.update_text(s))
        else:
            self.algorithm_service.run_algorithm(
                start, goal, map, slides, dijkstra=dijkstra, jps=jps, visualization=False)
        self.update_text(slides)
    ...
    #        self.window.after((idx - start) * 400, lambda s=slide: self.update_text(s))

    def animate(self, start=0):
        self.stop_animation()  # Stop any ongoing animation
        self.job_ids = []  # Reset job IDs list
        for idx, slide in enumerate(self.slides[start:], start=start):
            job_id = self.window.after((idx - start) * 400, lambda s=slide, c=idx: self.update_text_and_counter(s, c))
    ...
    def update_text_and_counter(self, slide, counter):
        ...
        self.lbl_update_counter.config(text=f"Browse slides from 0 to {self.max_counter}, current is {counter}")
        self.counter_entry.delete(0, tk.END)
    ...
    def stop_animation(self):
        for job_id in self.job_ids:
            self.window.after_cancel(job_id)  # Cancel the scheduled job
